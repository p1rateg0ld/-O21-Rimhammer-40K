...
{
    public class HediffOrkHealing : HediffWithComps
    {
        ...
        {
            using(IEnumerator<BodyPartRecord> enumerator = this.pawn.GetFirstMatchingBodyparts(this.pawn.RaceProps.body.corePart,HediffDefOf.MissingBodyPart, OrkHediffDefOf.OrkProtoBodypart, (Hediff hediff) => hediff is Hediff_AddedPart).GetEnumerator())
            {
                while (enumerator.MoveNext())
                {
                    ...
                    if (flag)
                    {
                        ...
                        this.pawn.health.AddHediff(OrkHediffDefOf.OrkProtoBodypart, part, null, null);
