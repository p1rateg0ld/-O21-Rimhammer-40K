...
{
    public class HediffNecrodermisRepair : HediffWithComps
    {
        ...
        {
            using (IEnumerator<BodyPartRecord> enumerator = this.pawn.GetFirstMatchingBodyparts(this.pawn.RaceProps.body.corePart, HediffDefOf.MissingBodyPart, NecronHediffDefOf.NecronProtoBodypart, (Hediff hediff) => hediff is Hediff_AddedPart).GetEnumerator())
            {
                while (enumerator.MoveNext())
                {
                    ...
                    if (flag)
                    {
                        ...
                        this.pawn.health.AddHediff(NecronHediffDefOf.NecronProtoBodypart, part, null, null);
        ...
        {
            this.ticksUntilNextHeal = Current.Game.tickManager.TicksGame + 1000;
        }
